-- Final Query incorporating:
-- 1. Length of Stay calculated as the count of distinct calendar days spanned by the ICU stay.
-- 2. Restraint metric calculated as the count of distinct calendar days with restraint documentation.
-- THIS VERSION IMPLEMENTS ALTERNATIVE DEFINITION 1: CONSERVATIVE/STRICT KEYWORDS

WITH first_icu_stay AS (
  -- Selects the first ICU stay per patient, including intime, outtime, hadm_id, and original los
...
-- MODIFIED FOR ALTERNATIVE DEFINITION 1: More Conservative/Strict Keyword Definition
restraint_charttimes AS (
  SELECT DISTINCT
    ce.subject_id,
    ce.stay_id,
    icu.intime, -- Added intime here (needed for first_restraint_time_cte potentially)
  ...
  REGEXP_CONTAINS(LOWER(value), r'(?i)(\bphysical restraint\b|\brestraints?\b applied|\b4[- ]point restraint\b|\bfour[- ]point restraint\b|\bleather restraint\b)') -- CONSERVATIVE KEYWORDS
  AND ce.charttime BETWEEN icu.intime AND icu.outtime -- Ensures restraint is within stay
),

-- Find the time of the very FIRST restraint event per stay (Needed for death_within_24h calculation)
...
-- Counts distinct CALENDAR DAYS with restraint documentation
calendar_restraint_days_count AS (
  SELECT
    subject_id,
    stay_id,
    -- Count distinct calendar dates where restraint occurred
    COUNT(DISTINCT DATE(restraint_time)) AS restraint_calendar_days
  FROM restraint_charttimes
  GROUP BY subject_id, stay_id
),
...
chem_restraint_inputs_day1 AS (
    ...
    WHERE inp.itemid IN (221744, 225974, 225975, 222168, 225942, 221662, 221385, 221668, 223773, 221468) -- itemids for Propofol, Dexmedetomidine, Midazolam, Lorazepam, Diazepam, Haloperidol, Olanzapine, Ziprasidone, Ketamine, Diphenhydramine (Check these are correct / what you intended)
...
SELECT
  -- Identifiers / Context
  pi.subject_id, icu.stay_id, icu.hadm_id,
  -- *** MODIFIED: Length of stay based on COUNT of Calendar Days Spanned ***
  (DATE_DIFF(DATE(icu.outtime), DATE(icu.intime), DAY) + 1) AS length_of_stay_calendar_days,
  icu.first_careunit AS icu_type,
  pi.anchor_year_group AS cohort_year,
  ...
  -- Physical restraint outcomes based on CALENDAR days
  CASE WHEN crd.stay_id IS NOT NULL THEN 1 ELSE 0 END AS physically_restrained, -- Primary outcome: Ever physically restrained (on any calendar day)? (0 or 1)
  COALESCE(crd.restraint_calendar_days, 0) AS restraint_calendar_days, -- Secondary outcome: Count of distinct calendar days with restraint (0, 1, 2...)
  -- Death within 24h of first restraint (logic unchanged, uses first_restraint_time)
  COALESCE(CASE
...
LEFT JOIN calendar_restraint_days_count crd ON icu.stay_id = crd.stay_id -- Join for count of distinct calendar days with restraint
LEFT JOIN first_restraint_time_cte frt ON icu.stay_id = frt.stay_id -- Join for first restraint time (still needed for death outcome)
...
WHERE icu.rn = 1 AND icu.los >= 1.0; -- Only first ICU stays >= 1 day (uses original fractional LOS for filtering)
